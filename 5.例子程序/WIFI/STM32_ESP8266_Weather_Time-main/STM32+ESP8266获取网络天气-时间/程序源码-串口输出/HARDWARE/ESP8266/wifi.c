...
/////////////////////////////////////////////////////
/////////////////////////////////////////////////////
signed ESP8266_Get_Net_Time(void)
{
	u8 ret;
	ret = UART3_Send_AT_Command("AT+CWJAP=\"HUAWEI\",\"qqqqqqqq\"\r\n","OK",3,100);  //设置接入点名称 密码  通道号  WPA2_PSK
	if(ret == 0)
	{
		return -3;
	}
	
	ret = UART3_Send_AT_Command("AT+CIPSTART=\"TCP\",\"cgi.im.qq.com\",80\r\n","OK",3,100); //设置为多连接
	if(ret == 0)
	{
		return -4;
	}
	
	ret = UART3_Send_AT_Command("AT+CIPSEND=100\r\n","OK",3,100);  //设置模块为TCP服务器，端口号为5000
	if(ret == 0)
	{
		return -5;
	}
...
signed ESP8266_wifi_Init(void)
{
	...
	ret = UART3_Send_AT_Command("AT+CWMODE=3\r\n","OK",3,100);   //设置模块为AP模式
	if(ret == 0)
	{
		return -2;
	}
	
	ret = UART3_Send_AT_Command("AT+RST\r\n","OK",3,100);  //设置接入点名称 密码  通道号  WPA2_PSK
	if(ret == 0)
	{
		return -3;
	}
